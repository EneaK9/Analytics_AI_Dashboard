...
// Helper functions
const createQueryKey = (type: string, platform: string, dateRange: DateRange, additionalParams?: Record<string, any>) => {
  const key = [type, platform, dateRange.preset || 'custom', dateRange.startDate, dateRange.endDate];
  if (additionalParams) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SalesMetrics> => {
      const params: Record<string, any> = {
        platform: filters.platform === 'combined' ? undefined : filters.platform,
...
export const useTrendAnalysis = (filters: DashboardFilters) => {
  const queryKey = createQueryKey('trends', filters.platform, filters.dateRange);
...
export const useAlertsData = (filters: DashboardFilters) => {
  const queryKey = createQueryKey('alerts', filters.platform, filters.dateRange);
 ...
 */
export const useSKUData = (filters: DashboardFilters, page: number = 1, pageSize: number = 50) => {
  const queryKey = createQueryKey('sku', filters.platform, filters.dateRange, { page, pageSize });
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SKUData> => {
      ...
      return {
        ...
        summary_stats: response.data?.summary_stats || response.data?.sku_inventory?.summary_stats,
...
export const useDashboardData = (filters: DashboardFilters) => {
  ...
  const isLoading = salesMetrics.isLoading || trendAnalysis.isLoading || alertsData.isLoading || skuData.isLoading;
  const error = salesMetrics.error || trendAnalysis.error || alertsData.error || skuData.error;
  ...
  return {
    ...
    refetch: useCallback(() => {
      ...
      alertsData.refetch();
      skuData.refetch();
    }, [salesMetrics.refetch, trendAnalysis.refetch, alertsData.refetch, skuData.refetch]),
  };
};
...
export const useDateRange = (initialPreset: '7d' | '30d' | '90d' = '7d') => {
  const dateRange = useMemo(() => getDateRangeFromPreset(initialPreset), [initialPreset]);
  
  const setPreset = useCallback((preset: '7d' | '30d' | '90d') => {
    return getDateRangeFromPreset(preset);
  }, []);
  
  const setCustomRange = useCallback((startDate: string, endDate: string): DateRange => {
    return {
      startDate,
      endDate,
      preset: 'custom',
...
export const usePrefetchDashboardData = () => {
  ...
  const prefetchPlatformData = useCallback(async (platform: 'shopify' | 'amazon', preset: '7d' | '30d' | '90d' = '7d') => {
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: async () => {
          const response = await api.get('/dashboard/inventory-analytics', {
            params: { platform, start_date: dateRange.startDate, end_date: dateRange.endDate, fast_mode: true }
          });
          return response.data?.inventory_analytics?.sales_kpis || {};
        },
      }),

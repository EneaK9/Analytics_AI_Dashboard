...
export const useInventoryData = (options: UseInventoryDataOptions = {}): InventoryDataState => {
  ...
  // Import the global store from the top level to avoid import issues
  const { useGlobalDataStore } = require('../store/globalDataStore');
  
  // Use global store data instead of local state - prevents duplicate API calls
  const inventoryAnalytics = useGlobalDataStore((state: any) => state.inventoryData);
  const skuData = useGlobalDataStore((state: any) => state.skuData || []); // Ensure always an array
  const loading = useGlobalDataStore((state: any) => state.loading.inventoryData || state.loading.skuData);
  const error = useGlobalDataStore((state: any) => state.errors.inventoryData || state.errors.skuData);
  
  // Extract nested data from inventoryAnalytics
  const salesKPIs = inventoryAnalytics?.inventory_analytics?.sales_kpis || null;
  const trendAnalysis = inventoryAnalytics?.inventory_analytics?.trend_analysis || null;
  const alertsSummary = inventoryAnalytics?.inventory_analytics?.alerts_summary || null;
  
  // Generate summary stats from SKU data if not provided
  const summaryStats = useMemo(() => {
    // Ensure skuData is an array before using array methods
    if (!skuData || !Array.isArray(skuData) || skuData.length === 0) return null;
    ...
    return {
      ...
      overstock_count: skuData.filter((sku: any) => sku.current_availability > 100).length,
    };
  }, [skuData]);

...
export const useGlobalDataStore = create<GlobalDataState>()((set, get) => ({
	...
	fetchInventoryData: async (forceRefresh = false) => {
		...
		// ⚡ ENHANCED: Include platform and refresh in key to prevent duplicate identical requests
		const requestKey = `inventoryData-${platform}-${forceRefresh}`;

		// Check if identical request is already in progress
		if (state.activeRequests.has(requestKey)) {
			console.log(`⏳ Identical request already in progress: ${requestKey}`);
			return;
		}

		// Check if data is still fresh (unless forcing refresh)
		if (!forceRefresh && !isDataStale(state.lastFetched.inventoryData)) {
		...
		try {
			...
			console.log(`🚀 Starting parallel API request: ${requestKey}`, params);
			...
			console.log(`✅ Parallel request completed: ${requestKey}`);
		} catch (error: any) {
			console.error(`❌ Parallel request failed: ${requestKey}`, error);
			set((state) => ({
				errors: {
					...state.errors,
	...
	fetchSKUData: async (page = 1, forceRefresh = false) => {
		const state = get();
		const { platform } = state.filters;
		// ⚡ ENHANCED: Include platform in key for better deduplication
		const requestKey = `skuData-${platform}-${page}-${forceRefresh}`;

		// Check if identical request is already in progress
		if (state.activeRequests.has(requestKey)) {
			console.log(
				`⏳ Identical SKU request already in progress: ${requestKey}`
			);
			return;
		}

		// Check if data is still fresh (unless forcing refresh)
		if (!forceRefresh && !isDataStale(state.lastFetched.skuData)) {
			console.log(`⚡ Using cached SKU data for: ${requestKey}`);
			return;
		}
		...
		try {
			...
			console.log(`🚀 Starting parallel SKU request: ${requestKey}`, params);
			...
			console.log(`✅ Parallel SKU request completed: ${requestKey}`);
		} catch (error: any) {
			console.error(`❌ Parallel SKU request failed: ${requestKey}`, error);
			set((state) => ({
				errors: {
					...state.errors,

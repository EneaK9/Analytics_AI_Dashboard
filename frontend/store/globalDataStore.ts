...
export const useGlobalDataStore = create<GlobalDataState>()((set, get) => ({
    ...
    setSmartDefaultFilters: () => {
      ...
      if (availableDataTypes.length === 0) {
        console.warn(`⚠️ No available data types for platform ${preferredPlatform}`);
        return;
      }
      ...
      console.log(`🎯 Setting defaults to platform: ${preferredPlatform}, dataType: ${preferredDataType}`);

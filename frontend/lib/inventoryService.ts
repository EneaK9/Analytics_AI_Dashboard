...
export interface SalesKPIs {
	total_sales_7_days: { revenue: number; units: number; orders: number };
	total_sales_30_days: { revenue: number; units: number; orders: number };
	total_sales_90_days: { revenue: number; units: number; orders: number };
...
class InventoryService {
	...
	async getPaginatedSKUInventory(
		...
		try {
			console.log(
				`🔍 Fetching paginated SKU inventory (page ${page}, size ${pageSize}, platform: ${platform})...`
			);
			...
			if (response.data && response.data.success) {
				...
				throw new Error(
					response.data?.error || "Failed to fetch paginated SKU data"
				);
			}
		} catch (error: any) {
	...
	async getSKUInventory(platform: string = "shopify"): Promise<SKUData[]> {
		try {
			...
			const response = await this.getPaginatedSKUInventory(
				1,
				100,
				true,
				false,
	...
	async isInventoryAnalyticsAvailable(): Promise<boolean> {
		try {
			...
			return (
				response.success &&
				response.inventory_analytics.data_summary.total_records > 0
			);
		} catch (error) {
			return false;
		}
	...
	async getInventorySummary(): Promise<{
		...
		try {
			...
			const totalValue =
				analytics.sku_inventory.summary_stats.total_inventory_value;
			const alertsCount = analytics.alerts_summary.summary_counts?.total_alerts;

			// Calculate stock health score (0-100)
			const lowStockCount =
				analytics.sku_inventory.summary_stats.low_stock_count;
			const outOfStockCount =
				analytics.sku_inventory.summary_stats.out_of_stock_count;
			const healthyStock = totalSKUs - lowStockCount - outOfStockCount;
			const stockHealthScore =
				totalSKUs > 0 ? Math.round((healthyStock / totalSKUs) * 100) : 100;

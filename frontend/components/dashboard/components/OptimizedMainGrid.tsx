...
const OptimizedMainGrid: React.FC<OptimizedMainGridProps> = memo(
	({ dashboardData, user, dashboardType = "main", className = "" }) => {
		const { prefetchPlatformData } = usePrefetchDashboardData();

		// Add delay before prefetching to ensure auth is stable
		useEffect(() => {
			const prefetchData = async () => {
				try {
					// Minimal delay to prevent race condition with auth
					await new Promise((resolve) => setTimeout(resolve, 300)); // Minimal delay
					...
					// Don't throw error - this is non-critical background prefetch
				}
			};
		...
		const handleComponentError = useCallback(
			(componentName: string, error: any) => {
							...
							<CardContent>
								<Box
									...
									<Box>
										...
										<Typography variant="body2" sx={{ opacity: 0.9 }}>
											React Query prevents duplicate API calls and provides
									...
									<Box>
										...
										<Typography variant="body2" sx={{ opacity: 0.9 }}>
											React.memo and optimized hooks prevent unnecessary

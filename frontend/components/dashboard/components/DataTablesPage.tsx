...
export default function DataTablesPage({
	...
	// State for date-filtered data - ALL HOOKS MUST BE CALLED BEFORE ANY EARLY RETURNS
	...
	const [dateFilterError, setDateFilterError] = React.useState<string | null>(
		null
	);
	...
	// Debounced row search (per-table) - must be before early returns
	React.useEffect(() => {
		const h = setTimeout(() => setTableSearch(tableSearchInput.trim()), 300);
		return () => clearTimeout(h);
	}, [tableSearchInput]);

	// Search highlight state - must be before early returns
	React.useEffect(() => {
		const handler = (e: any) => {
	...
	const validTables = (activeData?.tables || []).filter(
		(table: any) =>
			table?.columns &&
	...
	return (
		<Box sx={{ p: 3 }}>
			...
			<Box sx={{ width: "100%" }}>
				...
				{(() => {
					const table =
						validTables[Math.min(activeTab, validTables.length - 1)];
					return (
						<Card sx={{ height: "100%" }}>
							<CardHeader
								title={(() => {
												...
												.filter((row: any) => {
													...
													try {
														if (Array.isArray(row))
															return row.some((c: any) =>
																String(c ?? "")
																	.toLowerCase()
																	.includes(q)
															);
														if (row && typeof row === "object")
															return Object.values(row).some((c: any) =>
																String(c ?? "")
																	.toLowerCase()
																	.includes(q)
															);

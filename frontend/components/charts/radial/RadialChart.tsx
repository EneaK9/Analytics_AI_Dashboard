...
export default function RadialChart({
	...
	const processRadialData = (inputData: any[]) => {
		...
		if (
			inputData.length > 0 &&
			inputData[0].hasOwnProperty("name") &&
			...
			const valueKeys = keys.filter(
				(key) =>
					...
					(typeof inputData[0][key] === "number" ||
						!isNaN(Number(inputData[0][key])))
			);

			if (valueKeys.length > 0) {
				// Take first 4-5 items for radial chart
				const limitedData = inputData.slice(0, 5);
				const values = limitedData.map(
					(item) => Number(item[valueKeys[0]]) || 0
				);
				const total = values.reduce((sum, val) => sum + val, 0);
		...
		if (inputData.length > 0 && typeof inputData[0] === "number") {
			const series = inputData.slice(0, 5).map((value) => {
				...
				const result = num > 100 ? Math.min(num / 10, 100) : num;
	...
	const options: ApexOptions = {
		...
		tooltip: {
			...
			custom: function ({ series, seriesIndex, dataPointIndex, w }) {
				...
				const label =
					dataPointIndex !== null ? w.globals.labels[dataPointIndex] : null;
						...
						<div class="text-xs mt-1 text-gray-600">
							...
							${
								...
								typeof rawValue === "number"
									? `<div>Raw Value: <span class="font-medium text-gray-800">${rawValue.toLocaleString()}</span></div>`
									: ""
							}
						</div>
					</div>

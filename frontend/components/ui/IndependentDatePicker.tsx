...
const IndependentDatePicker: React.FC<IndependentDatePickerProps> = memo(
	({
		...
		return (
			<div
				className={`flex items-center gap-3 p-3 rounded-lg border ${getPlatformColor()} ${className}`}>
				{/* Platform indicator */}
				...
				{dateRange.preset && (
					<div className="text-xs text-gray-500 bg-white px-2 py-1 rounded border">
						Last {presetOptions.find((p) => p.value === dateRange.preset)?.days}{" "}
						days
					</div>
				)}
			</div>

...
import { useSalesMetrics, DateRange, DashboardFilters } from '../../hooks/useOptimizedDashboardData';
...
const OptimizedSmartEcommerceMetrics: React.FC<OptimizedSmartEcommerceMetricsProps> = memo(({
  ...
  const { data: salesKPIs, isLoading, error, refetch, isRefetching } = useSalesMetrics(filters);
    ...
    <div className={`space-y-4 ${className}`}>
              ...
              <div className="mt-4 flex items-end justify-between">
                ...
                <span
                  className={`flex items-center gap-1 text-sm font-medium ${
                    metric.changeType === "increase" ? "text-meta-3" : "text-meta-5"
                  }`}>
                  {metric.change}
      ...
      <div className="text-center mt-4">
        <p className="text-xs text-gray-500">
          {platform === 'combined' ? 'All Platforms' : platform.charAt(0).toUpperCase() + platform.slice(1)} metrics • 

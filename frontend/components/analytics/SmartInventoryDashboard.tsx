...
export default function SmartInventoryDashboard({
	...
	// ⚡ OPTIMIZED: Single API call for ALL platforms - no more individual requests!
	const {
		loading,
		error,
		shopifyData,
	...
	// Extract platform-specific data based on selected platform
	const platformData =
		selectedPlatform === "shopify" ? shopifyData : amazonData;
	const skuData = platformData?.sku_data || [];
	const summaryStats = platformData?.sku_summary_stats;
	const salesKPIs = platformData?.sales_kpis;
	const trendAnalysis = platformData?.trend_analysis;
	const alertsSummary = platformData?.alerts_summary;
						...
						<CardContent>
							...
							{salesKPIs.total_sales_7_days?.change_percentage && (
								<p className="text-sm text-gray-600 mt-1">
									{salesKPIs.total_sales_7_days.change_percentage} vs last
									period
								</p>
							)}
						</CardContent>
					...
					<Card>
						...
						<CardContent>
							...
							{salesKPIs.total_sales_30_days?.change_percentage && (
								<p className="text-sm text-gray-600 mt-1">
									{salesKPIs.total_sales_30_days.change_percentage} vs last
									period
								</p>
							)}
						</CardContent>
					...
					<Card>
						...
						<CardContent>
							...
							{salesKPIs.average_order_value?.change_percentage && (
								<p className="text-sm text-gray-600 mt-1">
									{salesKPIs.average_order_value.change_percentage} vs last
									period
								</p>
							)}
						</CardContent>
				...
				<Card className="border-orange-200">
					...
					<CardContent>
						<div className="space-y-2">
							{alertsSummary.low_stock_alerts
								.slice(0, 3)

...
async def test_shopify_permissions():
    ...
    try:
        ...
        if not success:
            print("   ⚠️ Cannot continue without valid connection")
            return
                ...
                try:
                    ...
                    async with session.get(url, headers=connector.headers) as response:
                        if response.status == 200:
                            ...
                            permissions[description] = f"⚠️ HTTP {response.status}"
                            print(f"   {description}: ⚠️ HTTP {response.status}")
        ...
        print("   ⚠️ Note: Shopify allows 40 requests per second (burst)")
...
async def test_amazon_permissions():
    ...
    print("\n⚠️ Note: Amazon SP-API has strict rate limits:")
...
if __name__ == "__main__":
    ...
    print("⚠️  Note: This test requires the backend environment with all dependencies")

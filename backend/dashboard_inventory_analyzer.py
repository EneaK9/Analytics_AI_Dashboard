...
class DashboardInventoryAnalyzer:
    ...
    async def get_dashboard_inventory_analytics(self, client_id: str, platform: str = "shopify") -> Dict[str, Any]:
        ...
        try:
            ...
            analytics = {
                ...
                "trend_analysis": await self._get_trend_visualizations(client_id, shopify_data, amazon_data, start_date, end_date),
    ...
    async def _get_sku_list(self, client_id: str, shopify_data: Dict, amazon_data: Dict, page: int = 1, page_size: int = 50, use_cache: bool = True, platform: str = "shopify") -> Dict[str, Any]:
    ...
    async def _get_kpi_charts(self, client_id: str, shopify_data: Dict, amazon_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate sales for each period with proper date filtering (platform-specific)
            all_orders = shopify_orders + amazon_orders  # This will only have one platform's data based on the fetch logic above
            ...
            try:
                ...
                if created_at:
                    ...
                    if start_date <= order_date <= end_date:
                        ...
                        # Extract actual units from line items
                        order_units = 0
                        
                        # Try to extract from raw_data first (for Shopify orders)
                        ...
                        if raw_data:
                            try:
                                ...
                                if isinstance(line_items, list) and len(line_items) > 0:
                                    ...
                                    order_units = int(order.get('line_items_count', 1) or 1)
                            except Exception:
                                # Fallback to line_items_count if parsing fails
                                order_units = int(order.get('line_items_count', 1) or 1)
                        else:
                            # No raw_data, try other fields (prioritize quantity for Amazon)
                            if 'quantity' in order:
                                order_units = int(order.get('quantity', 1) or 1)
                            elif 'line_items_count' in order:
                                order_units = int(order.get('line_items_count', 1) or 1)
                            elif 'number_of_items_shipped' in order:
                                order_units = int(order.get('number_of_items_shipped', 1) or 1)
                            else:
                                order_units = 1
                        
                        # Ensure we always count at least 1 unit per order
                        if order_units == 0:
                            order_units = 1
    ...
    async def _get_trend_visualizations(self, client_id: str, shopify_data: Dict, amazon_data: Dict, start_date: Optional[str] = None, end_date: Optional[str] = None) -> Dict[str, Any]:
        """Get trend visualization data with date range support"""
        try:
            shopify_orders = shopify_data.get('orders', [])
            amazon_orders = amazon_data.get('orders', [])
            all_orders = shopify_orders + amazon_orders
            
            logger.info(f"📈 Generating trends for {len(all_orders)} total orders with date range: {start_date} to {end_date}")
            ...
            if start_date and end_date:
                ...
                # Calculate historical period (same length, going backwards from start_date)
                historical_end_dt = start_dt - timedelta(days=1)
                historical_start_dt = historical_end_dt - timedelta(days=period_length - 1)
                
                logger.info(f"📊 TREND PERIODS:")
                logger.info(f"   Current: {start_dt.strftime('%Y-%m-%d')} to {end_dt.strftime('%Y-%m-%d')} ({period_length} days)")
                logger.info(f"   Historical: {historical_start_dt.strftime('%Y-%m-%d')} to {historical_end_dt.strftime('%Y-%m-%d')} ({period_length} days)")
                
                # Calculate sales for both periods
                current_sales = self._calculate_sales_for_period(all_orders, start_dt, end_dt)
                historical_sales = self._calculate_sales_for_period(all_orders, historical_start_dt, historical_end_dt)
                
                # Generate weekly data for the current period and preceding weeks for chart
                weekly_data = []
                chart_start = historical_start_dt
                chart_end = end_dt
                
                # Generate weekly data covering both historical and current periods
                current_week_start = chart_start
                while current_week_start <= chart_end:
                    week_end = min(current_week_start + timedelta(days=6), chart_end)
                    week_sales = self._calculate_sales_for_period(all_orders, current_week_start, week_end)
                    
                    weekly_data.append({
                        "week": current_week_start.strftime("%Y-W%U"),
                ...
                current_avg_revenue = current_sales['revenue'] / period_length if period_length > 0 else 0
                historical_avg_revenue = historical_sales['revenue'] / period_length if period_length > 0 else 0
                
                current_avg_units = current_sales['units'] / period_length if period_length > 0 else 0
                historical_avg_units = historical_sales['units'] / period_length if period_length > 0 else 0

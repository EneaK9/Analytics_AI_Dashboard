...
class DashboardInventoryAnalyzer:
    ...
    async def _get_kpi_charts(self, client_id: str, shopify_data: Dict, amazon_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # Determine platform based on the data provided

            has_shopify_data = len(shopify_data.get('products', [])) > 0 or len(shopify_data.get('orders', [])) > 0

            has_amazon_data = len(amazon_data.get('products', [])) > 0 or len(amazon_data.get('orders', [])) > 0

            

            if has_shopify_data and has_amazon_data:

                platform = "combined"

            elif has_shopify_data:
                ...
                platform = "combined"  # Default fallback

            

            logger.info(f" Using component_data_functions for consistent 30-day metrics for client {client_id} (platform: {platform})")
            ...
            total_sales_task = asyncio.create_task(
                ...
                self.component_data.get_total_sales_data(client_id, platform, start_date, end_date)

            )

            inventory_turnover_task = asyncio.create_task(

                self.component_data.get_inventory_turnover_data(client_id, platform, start_date, end_date)

            )

            days_of_stock_task = asyncio.create_task(

                self.component_data.get_days_of_stock_data(client_id, platform, start_date, end_date)

            )

            units_sold_task = asyncio.create_task(

                self.component_data.get_units_sold_data(client_id, platform, start_date, end_date)

            )

            inventory_levels_task = asyncio.create_task(

                self.component_data.get_inventory_levels_data(client_id, platform, start_date, end_date)
            ...
            # Extract the consistent 30-day metrics based on platform

            # Component functions return data differently based on platform:

            # - For single platforms ("shopify", "amazon"): data is under platform key

            # - For "combined": data is returned directly
            ...
            if platform == "combined":
                ...
                # For single platforms, data is under platform key

                platform_sales = total_sales_data.get(platform, {})

                platform_turnover = inventory_turnover_data.get(platform, inventory_turnover_data)

                platform_days_stock = days_of_stock_data.get(platform, {})

                platform_units_sold = units_sold_data.get(platform, {})

                platform_inventory = inventory_levels_data.get(platform, {})
            ...
            return {
                ...
                "total_sales_30_days": {
                    ...
                    "revenue": platform_sales.get('total_revenue', 0),

                    "units": platform_units_sold.get('total_units_sold_30d', 0),

                    "orders": platform_sales.get('total_orders', 0)

                },

                "inventory_turnover_30_days": {

                    "turnover_rate": platform_turnover.get('inventory_turnover_ratio', 0),

                    "comparison": platform_turnover.get('turnover_comparison', {}),

                    "avg_days_to_sell": platform_turnover.get('avg_days_to_sell', 999),

                    "fast_moving_items": platform_turnover.get('fast_moving_items', 0)

                },

                "days_of_remaining_stock": {

                    "avg_days_of_stock": platform_days_stock.get('avg_days_of_stock', 999),

                    "daily_sales_velocity": platform_days_stock.get('daily_sales_velocity', 0),

                    "current_inventory": platform_days_stock.get('current_inventory', 0),

                    "low_stock_count": platform_days_stock.get('low_stock_count', 0),

                    "out_of_stock_count": platform_days_stock.get('out_of_stock_count', 0),

                    "overstock_count": platform_days_stock.get('overstock_count', 0)

                },

                "inventory_levels_chart": platform_inventory.get('inventory_levels_chart', []),

                "units_sold_30_days": {

                    "total_units_sold": platform_units_sold.get('total_units_sold_30d', 0),

                    "units_sold_chart": platform_units_sold.get('units_sold_chart', []),

                    "velocity_metrics": platform_units_sold.get('velocity_metrics', {})

                },

                "total_inventory_units": platform_inventory.get('current_total_inventory', 0),
    ...
    async def _get_trend_visualizations(self, client_id: str, shopify_data: Dict, amazon_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f" Using component_data_functions for consistent trend analysis for client {client_id} (platform: {platform})")
            ...
            historical_comparison_task = asyncio.create_task(
                ...
                self.component_data.get_historical_comparison_data(client_id, platform, start_date, end_date)
            ...
            # Extract the consistent trend data based on platform
            ...
            if platform == "combined":
                ...
                platform_historical = historical_comparison_data.get(platform, {})
            ...
            return {
                ...
                "inventory_levels_chart_30_days": platform_inventory.get('inventory_levels_chart', []),

                "units_sold_chart_30_days": platform_units_sold.get('units_sold_chart', []),
                ...
                "velocity_metrics_30_days": platform_units_sold.get('velocity_metrics', {}),

    ...
    async def _get_units_sold_in_period(
        ...
        try:
            ...
            if platform == "amazon":
                ...
                # For combined platform: Get units from both Shopify and Amazon



                shopify_units = await self._get_units_sold_in_period(

                    client_id, "shopify", start_date, end_date

                )



                amazon_units = await self._get_units_sold_in_period(

                    client_id, "amazon", start_date, end_date

                )



                total_units = shopify_units + amazon_units



                logger.info(f" Combined units sold: Shopify={shopify_units}, Amazon={amazon_units}, Total={total_units}")
    ...
    async def _get_current_inventory(self, client_id: str, platform: str) -> int:
        ...
        try:
            ...
            if platform == "shopify":
                ...
                # For combined platform: Get inventory from both Shopify and Amazon
                ...
                logger.info(f" Combined current inventory: Shopify={shopify_inventory}, Amazon={amazon_inventory}, Total={total_inventory}")
